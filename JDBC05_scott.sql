...
--○ 데이터 입력
-- 강원 경기 경남 경북 부산 서울 인천 전남 전북 제주 충남 축북
...
INSERT INTO TBL_JIKWI(JIKWI_ID, JIKWI_NAME, MIN_BASICPAY) VALUES(JIKWISEQ.NEXTVAL, '상사', 3500000);
...
ADD (CONSTRAINT EMP_EMP_ID_PK PRIMARY KEY(EMP_ID)
    ,CONSTRAINT EMP_CITY_ID_PK FOREIGN KEY(CITY_ID) REFERENCES TBL_CITY(CITY_ID)
    ,CONSTRAINT EMP_BUSEO_ID_PK FOREIGN KEY(BUSEO_ID) REFERENCES TBL_BUSEO(BUSEO_ID)
    ,CONSTRAINT EMP_JIKWI_ID_PK FOREIGN KEY(JIKWI_ID) REFERENCES TBL_JIKWI(JIKWI_ID)    
    ); 
...
--------------------------------------------------
----------------------                                  ▼
INSERT INTO TBL_EMP(EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG) VALUES (EMPSEQ.NEXTVAL, '%s', '%s', '%s', (SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC='%d),'%s',(SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '%d'),(SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '%d'),%d,%d)

SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC='%d'
;

SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '%d'
;

SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '%d'
;
...
-- 사번 이름 주민번호 입사일 지역이름 전화번호 부서이름 직위이름 기본급 수당
...
-->한줄구성
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG FROM EMPVIEW ORDER BY EMP_ID
;
...
/*
INSERT INTO TBL_EMP(EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG) 
VALUES (EMPSEQ.NEXTVAL, '%s', '%s', '%s', (SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC='%d),'%s',(SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '%d'),(SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '%d'),%d,%d)
*/
...
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, NVL(TEL,'번호없음') AS TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW;
...
--11. 직원 데이터 수정 쿼리문
UPDATE TBL_EMP
SET EMP_NAME = '김민성', SSN = '970723-1234567'
...
--> 한줄구성
UPDATE TBL_EMP SET EMP_NAME = '김민성', SSN = '970723-1234567', IBSADATE = TO_DATE('2021-07-20','YYYY-MM-DD'), CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_NAME = '제주'), TEL = '010-5154-6322', BUSEO_ID = (SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부'), JIKWI_ID = (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '과장'), BASICPAY = 3000000, SUDANG = 2000000 WHERE EMP_ID = 100;
